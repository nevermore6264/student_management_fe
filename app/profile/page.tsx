                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
                    <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                        ...
                        <div className="text-center md:text-left flex-1">
                            <h1 className="text-3xl font-bold text-gray-900 mb-2">{profile.hoTenSinhVien}</h1>
                                ...
                                <div className="bg-gray-50 rounded-xl p-6">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <div className="text-gray-900 font-medium">{profile.hoTenSinhVien || '-'}</div>
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Mã sinh viên</label>
                                            <div className="text-gray-900 font-mono bg-gray-100 px-3 py-1 rounded-md inline-block">{profile.maSinhVien || '-'}</div>
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Mã lớp</label>
                                            <div className="text-gray-900 font-mono bg-gray-100 px-3 py-1 rounded-md inline-block">{profile.maLop || '-'}</div>
                                        </div>
                                        <div>
                ...
                {editForm && (
                    <div className="p-fluid grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="p-field">
                            <label htmlFor="hoTenSinhVien" className="block font-semibold mb-2 text-gray-700">Họ tên *</label>
                            <InputText
                                id="hoTenSinhVien"
                                className={`w-full p-3 border rounded-xl transition-all duration-200 ${editErrors.hoTenSinhVien ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200'}`}
                                value={editForm.hoTenSinhVien}
                                onChange={e => handleEditInput('hoTenSinhVien', e.target.value)}
                            />
                            {editErrors.hoTenSinhVien && <small className="text-red-500 text-sm mt-1 block">Họ tên không được để trống</small>}
                        </div>
                        <div className="p-field">
                        ...
                        <div className="p-field">
                            <label htmlFor="diaChi" className="block font-semibold mb-2 text-gray-700">Địa chỉ</label>
                            <InputText
                                id="diaChi"
                                className="w-full p-3 border border-gray-300 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200"
                                value={editForm.diaChi || ''}
                                onChange={e => handleEditInput('diaChi', e.target.value)}
                            />
                        </div>
                        <div className="p-field">
                            <label htmlFor="ngaySinh" className="block font-semibold mb-2 text-gray-700">Ngày sinh</label>
                            <InputText
                                id="ngaySinh"
                                type="date"
                                className="w-full p-3 border border-gray-300 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200"
                                value={editForm.ngaySinh || ''}
                                onChange={e => handleEditInput('ngaySinh', e.target.value)}
                            />
                        </div>
                        <div className="p-field">
                            <label htmlFor="gioiTinh" className="block font-semibold mb-2 text-gray-700">Giới tính</label>
                            <Dropdown
                                id="gioiTinh"
                                className="w-full"
                                value={editForm.gioiTinh}
                                options={genderOptions}
                                onChange={e => handleEditInput('gioiTinh', e.value)}
                                placeholder="Chọn giới tính"
                                style={{ borderRadius: '12px' }}
                            />
                        </div>
                        <div className="p-field">
                            <label htmlFor="maSinhVien" className="block font-semibold mb-2 text-gray-700">Mã sinh viên</label>
                            <InputText
                                id="maSinhVien"
                                className="w-full p-3 border border-gray-300 rounded-xl bg-gray-100 cursor-not-allowed"
                                value={editForm.maSinhVien}
                                disabled
                            />
                        </div>
                        <div className="p-field">
                            <label htmlFor="maLop" className="block font-semibold mb-2 text-gray-700">Mã lớp</label>
                            <InputText
                                id="maLop"
                                className="w-full p-3 border border-gray-300 rounded-xl bg-gray-100 cursor-not-allowed"
                                value={editForm.maLop}
                                disabled
                            />
                        </div>
                    </div>
